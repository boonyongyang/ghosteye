...
class ScriptHistoryService {
  ScriptHistoryService({LoadHistoryPreferencesFn? loadPreferences})
  ...
  Future<List<ScriptSession>> upsertSession(ScriptSession session) {
    return _writeMutex.protect(() async {
  ...
  Future<List<ScriptSession>> deleteSession(String sessionId) {
    return _writeMutex.protect(() async {
      final sessions = await loadSessions();
